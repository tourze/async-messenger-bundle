...
class MultiQueueIsolationTest extends RedisIntegrationTestCase
{
    ...
    public function test_delayedQueuesAreIsolated(): void
    {
        ...
        $messagesB = iterator_to_array($transports['queue_b']->get());
        ...
        foreach ($messagesB as $msg) {
            $this->assertEquals('delayed B', $msg->getMessage()->content);
            $transports['queue_b']->ack($msg);
        }
        ...
        // 现在 queue_a 应该有消息
        $messagesA = iterator_to_array($transports['queue_a']->get());
        ...
        foreach ($messagesA as $msg) {
            $this->assertEquals('delayed A', $msg->getMessage()->content);
            $transports['queue_a']->ack($msg);
        }
